...
const URL =
  process.env.URL || 'https://www.bilibili.com/read/readlist/rl60978/';
...
function collectAllSameOriginAnchorsDeep(sameOrigin = true) {
  ...
  const filtered = allElements
    ...
    .filter(e1 => !e1.href.includes('comment'))
    .filter(e1 => e1.href.includes('/?from=readlist'))
    // .filter((e1, index) => index < 3)
    .map(a => a.href);
...
async function crawl(browser, page, depth = 0) {
  ...
  if (crawledPages.has(page.url)) {
    ...
    if (SCREENSHOTS) {
      ...
      imgBuff = await sharp(imgBuff)
        .resize(null, 150)
        .toBuffer(); // resize image to 150 x auto.
...
const getPage = async (browser, links) => {
  for (let i = 0, len = links.length; i < len; i++) {
    ...
    const content = await page.evaluate(() => {
      let $ = window.$;
      let content = $('.article-holder').html().replace(/\/\/www/g,'https://www');
      return content;
    });
    await page.close();
...
(async () => {
  ...
  await getPage(browser, root.children);
